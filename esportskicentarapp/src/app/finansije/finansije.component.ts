...
export class FinansijeComponent implements OnInit {
  ...
  public totalIncome: number | undefined;
  ...
  loadIncomeExpensesData() {
    this.finansijeService.getPrihodiRashodiData().subscribe(
      (response: PrihodiRashodiStatsDto) => {
        this.totalIncome = response.prihodi;
        this.totalExpenses = response.rashodi;
      },
      (error: HttpErrorResponse) => {
        alert(error.message);
      }
